...
public class FaceEmbeddingExtractor {
    ...
    private static Bitmap preprocessFace(Bitmap input) {
        ...
        result.setPixels(pixels, 0, FACE_SIZE, 0, 0, FACE_SIZE, FACE_SIZE);        if (resized != input && !resized.isRecycled()) resized.recycle();
        return result;
    }
    ...
    private static void extractSimpleLBP(float[] gray, int w, int h, float[] out, int offset, int dims) {
        int grid = 4, cellW = w / grid, cellH = h / grid;
        ...
        for (int gy = 0; gy < grid; gy++) {
            for (int gx = 0; gx < grid; gx++) {
                ...
                for (int s = 0; s < dimsPerCell && s < 4; s++) {
                    int sx = (s % 2) * (cellW / 2), sy = (s / 2) * (cellH / 2);
                    int idx = cellOffset + sy * w + sx;
                    if (idx < gray.length) {
                        float center = gray[idx];
                        // ✅ FIX: pattern must be int for bitwise operations
                        int pattern = 0;
                        int[] dx = {1, 0, -1, 0}, dy = {0, 1, 0, -1};
                        for (int n = 0; n < 4; n++) {
                            int nx = Math.max(0, Math.min(w-1, (gx * cellW) + sx + dx[n]));
                            int ny = Math.max(0, Math.min(h-1, (gy * cellH) + sy + dy[n]));                            int nidx = ny * w + nx;
